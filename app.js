...
function toggleSearch() { document.getElementById('search-modal').classList.toggle('open'); }
function toggleSettingsModal() { document.getElementById('settings-modal').classList.toggle('open'); }
function toggleMusicModal() { document.getElementById('music-modal').classList.toggle('open'); }
function openHighlightPalette() { document.getElementById('highlight-modal').classList.add('open'); }
function closeHighlightPalette() { document.getElementById('highlight-modal').classList.remove('open'); }
...
function checkDarkMode() {
    if (localStorage.getItem('wog_dark_mode') === 'true') {
        document.body.classList.add('dark-mode');
        document.getElementById('theme-icon').innerText = 'light_mode';
    } else { document.getElementById('theme-icon').innerText = 'dark_mode'; }
}

// ---- Views & History API ----
function hideAllViews() { document.querySelectorAll('.view-container').forEach(v => v.style.display = 'none'); clearVerseSelection(); }
...
function showHome(pushState = true) { hideAllViews(); document.getElementById('home-view').style.display = 'block'; if(pushState) history.pushState({page: 'home'}, "Home", "?view=home"); }
function showBookmarksView(pushState = true) { hideAllViews(); document.getElementById('bookmarks-view').style.display = 'block'; renderBookmarks(); if(pushState) history.pushState({page: 'bookmarks'}, "Bookmarks", "?view=bookmarks"); }
function showNotesView(pushState = true) { hideAllViews(); document.getElementById('notes-view').style.display = 'block'; renderNotesList(); if(pushState) history.pushState({page: 'notes'}, "Notes", "?view=notes"); }
function showHistoryView(pushState = true) { hideAllViews(); document.getElementById('history-view').style.display = 'block'; renderHistory(); if(pushState) history.pushState({page: 'history'}, "History", "?view=history"); }
function showAboutView(pushState = true) { hideAllViews(); document.getElementById('about-view').style.display = 'block'; if(pushState) history.pushState({page: 'about'}, "About", "?view=about"); }
...
function populateSidebar() {
    ...
    otFiles.forEach(file => {
        ...
        btn.onclick = () => { toggleSidebar(); loadBookData(file, "1"); }; sidebarList.appendChild(btn);
    });
...
// ---- SMART NAVIGATOR LOGIC ----
function openBibleNav() {
    document.getElementById('bible-nav-modal').classList.add('open');
    renderNavBooks();
    switchNavTab('books');
    document.getElementById('tab-chapters').classList.add('disabled');
    document.getElementById('tab-verses').classList.add('disabled');
}

function closeBibleNav() { document.getElementById('bible-nav-modal').classList.remove('open'); }

function switchNavTab(tab) {
    document.querySelectorAll('.nav-tab').forEach(t => t.classList.remove('active'));
    document.getElementById(`tab-${tab}`).classList.add('active');

    document.querySelectorAll('.nav-content-area').forEach(c => {
        c.style.display = 'none'; c.classList.remove('chapter-grid'); 
    });

    const activeContainer = document.getElementById(`nav-${tab}-container`);
    activeContainer.style.display = 'block';
    if(tab === 'chapters' || tab === 'verses') { activeContainer.classList.add('chapter-grid'); }
}

function renderNavBooks() {
    const container = document.getElementById('nav-books-container');
    container.innerHTML = '';

    let otHeader = document.createElement('div'); otHeader.className = 'book-cat'; otHeader.innerText = 'పాత నిబంధన (OT)'; container.appendChild(otHeader);
    otFiles.forEach(file => {
        let btn = document.createElement('button'); btn.className = 'nav-book-btn'; let bName = file.replace('.json', '');
        btn.innerText = bName; if(tempNavFileName === file) btn.classList.add('active-book');
        btn.onclick = () => { selectNavBook(file, bName); }; container.appendChild(btn);
    });

    let ntHeader = document.createElement('div'); ntHeader.className = 'book-cat'; ntHeader.innerText = 'కొత్త నిబంధన (NT)'; container.appendChild(ntHeader);
    ntFiles.forEach(file => {
        let btn = document.createElement('button'); btn.className = 'nav-book-btn'; let bName = file.replace('.json', '');
        btn.innerText = bName; if(tempNavFileName === file) btn.classList.add('active-book');
        btn.onclick = () => { selectNavBook(file, bName); }; container.appendChild(btn);
...
async function selectNavBook(fileName, bookName) {
    tempNavFileName = fileName;
    document.getElementById('tab-chapters').innerText = "Loading...";

    if (!allBibleData[fileName]) {
        try {
            // Cache Bypass Trick to fix "Loading..." stuck issue
            let response = await fetch(fileName + '?v=' + new Date().getTime(), { cache: 'no-store' });
            if (!response.ok) throw new Error("Network Error");
            let data = await response.json();
            allBibleData[fileName] = data;
        } catch (error) { 
            console.log("Fetch Error:", error); 
            alert(`ఎర్రర్: ${fileName} కనుగొనబడలేదు. మీ నెట్వర్క్ లేదా ఫైల్స్ చెక్ చేయండి.`);
            document.getElementById('tab-chapters').innerText = "అధ్యాయం";
            return; 
        }
    }

    tempNavBookName = Object.keys(allBibleData[fileName])[0];
    document.getElementById('tab-chapters').innerText = "అధ్యాయం";
    document.getElementById('tab-chapters').classList.remove('disabled');
...
function renderNavChapters() {
    const container = document.getElementById('nav-chapters-container');
    container.innerHTML = '';
    const chapters = allBibleData[tempNavFileName][tempNavBookName];

    for (let chapterNum in chapters) {
        const btn = document.createElement('button'); btn.className = 'grid-num-btn'; btn.innerText = chapterNum;
        btn.onclick = () => { selectNavChapter(chapterNum); }; container.appendChild(btn);
    }
}

function selectNavChapter(chapterNum) {
    tempNavChapterNum = chapterNum;
    document.getElementById('tab-verses').classList.remove('disabled');
    renderNavVerses(); switchNavTab('verses');
}

function renderNavVerses() {
    const container = document.getElementById('nav-verses-container');
    container.innerHTML = '';

    const fullChapBtn = document.createElement('button');
    fullChapBtn.className = 'grid-num-btn';
    fullChapBtn.style.gridColumn = '1 / -1';
    fullChapBtn.style.backgroundColor = 'var(--primary)'; fullChapBtn.style.color = 'white';
    fullChapBtn.innerText = 'పూర్తి అధ్యాయం చదవండి';
    fullChapBtn.onclick = () => { closeBibleNav(); loadBookData(tempNavFileName, tempNavChapterNum); };
    container.appendChild(fullChapBtn);

    const verses = allBibleData[tempNavFileName][tempNavBookName][tempNavChapterNum];
    for (let verseNum in verses) {
        const btn = document.createElement('button');
        btn.className = 'grid-num-btn'; btn.innerText = verseNum;
        btn.onclick = () => { closeBibleNav(); loadBookData(tempNavFileName, tempNavChapterNum, verseNum); };
        container.appendChild(btn);
    }
}
// ------------------------------------
// Core Reading & Scroll Logic
// ------------------------------------
async function loadBookData(fileName, targetChapter = "1", targetVerse = null, highlightQuery = null) {
    currentFileName = fileName;
    hideAllViews();
    document.getElementById('reading-view').style.display = 'block';
    document.getElementById('reading-book-title').innerHTML = `Loading... <span class="material-icons-outlined">expand_more</span>`;

    if (!allBibleData[fileName]) {
        try {
            // Cache Bypass Trick - ఇది Loading లో ఆగిపోకుండా కాపాడుతుంది
            let response = await fetch(fileName + '?v=' + new Date().getTime(), { cache: 'no-store' });
            ...
            document.getElementById('reading-book-title').innerHTML = `లోడ్ అవ్వలేదు <span class="material-icons-outlined">expand_more</span>`;
            alert("డేటా లోడ్ అవ్వలేదు. దయచేసి నెట్వర్క్ చెక్ చేయండి లేదా ఫైల్ ఉందో లేదో చూడండి.");
            return;
        }
    }
...
function openChapterReading(chapterNum, targetVerse = null, highlightQuery = null, pushState = true) {
    ...
    // టైటిల్ అప్డేట్ (నొక్కితే మళ్ళీ స్మార్ట్ నావిగేటర్ ఓపెన్ అవుతుంది)
    document.getElementById('reading-book-title').innerHTML = `${currentBookName} ${chapterNum} <span class="material-icons-outlined">expand_more</span>`;
...
function shareSelectedVerses() {
    ...
    selectedVerses.forEach(v => { 
        ...
        if (v.enText && v.enText.trim() !== "") { shareTextStr += `(${v.enText})\n`; }
        shareTextStr += `\n`;
    });
...
async function shareTextFn(text, title) {
    if (navigator.share) {
        try { await navigator.share({ title: title, text: text }); return; } catch (err) { console.log('Share cancelled:', err); }
    } 
    try {
        ...
        try { document.execCommand('copy'); alert("టెక్స్ట్ కాపీ చేయబడింది! (Copied)"); } catch (e) { alert("షేర్/కాపీ చేయడం విఫలమైంది."); }
...
async function executeSearch() {
    ...
    document.getElementById('reading-book-title').innerHTML = `Searching... <span class="material-icons-outlined">expand_more</span>`;
    ...
    document.getElementById('reading-book-title').innerHTML = `${scopeTitle} ఫలితాలు (${foundCount}) <span class="material-icons-outlined">expand_more</span>`;
}
